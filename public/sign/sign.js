...
function LOGIN_Customer(_account) {
  ...
  fetch('https://nike0403.herokuapp.com/users/login', {
    method: "POST",
    headers: {
...
function signUp() {
  ...
  if (check == true) {
    ...
    let _gender = document.querySelector('.sign-up-form .gender input[name="gender"]:checked').value
    let _dob = new Date(document.querySelector('.sign-up-form .dob input').value)
...
function CREATE_Customer(_customer) {
  ...
  fetch('https://nike0403.herokuapp.com/users/create', {
    method: "POST",
    headers: {
      'Content-Type': 'application/json'
    },
